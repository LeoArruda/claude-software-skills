...
From [Filip Hráček’s benchmark](https://filiph.net/text/benchmarking-flutter-flame-unity-godot.html) (Flutter/Dart team), comparing Flutter/Flame with Unity and Godot:

### Setup
- Demo: “The Bench” — realistic-ish scene
- Includes: animated background, moving sprites, UI, background music
- Platforms: iOS and Web

### Comparison

| Metric | Flutter/Flame | Unity | Godot |
|--------|----------------|-------|-------|
| **Cold start** | Fastest | Slower | Slower |
| **Max entities** | ~hundreds | Thousands | Thousands |
| **CPU usage** | ~35–40% | ~35–40% | ~35–40% |
| **Memory (Web)** | Higher | Lower | Lower |
| **Memory (iOS)** | Lower | Medium | Medium |

### Bottleneck

Entity count is the main limiter:

```dart
// ⚠️ Performance drops past ~200–300 active entities
// Flutter’s render stack is not tuned like a dedicated game engine

// ✅ Mitigations
// 1. Pooling — less GC churn
// 2. Culling — update only visible entities
// 3. Batching — fewer draw calls
// 4. Lower tick rate — TimerComponent for non-critical objects
```

### Good fits

Based on that benchmark, Flame works well for:

| Good fit | Poor fit |
|----------|----------|
| Casual (card, puzzle) | Bullet-hell |
| Hyper-casual | RTS / huge battles |
| Visual novels / interactive story | Physics-heavy sims |
| Turn-based RPG / tactics | Particle-heavy effects |
| 2D platformer (moderate enemy count) | 3D games |
| Mini-games inside Flutter apps | AAA titles |

### Targets

| Entity count | Expected FPS | Guidance |
|--------------|----------------|----------|
| Under 50 | 60 | Little tuning needed |
| 50–150 | 60 | Basic pooling |
| 150–300 | 30–60 | Pooling + culling + batching |
| Over 300 | Under 30 | Consider Unity/Godot |
